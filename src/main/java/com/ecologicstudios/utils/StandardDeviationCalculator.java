...
/**
 * Utility for computing basic sample statistics (mean, sample variance, standard
 ...
 * - Variance uses the sample formula (divide by N-1).</p>
 */
public class StandardDeviationCalculator{
     ...
     *
     * @param graph list of {@link XYChart.Data} where the last element is the latest round;
     *              must contain at least three points (at least two baseline points + latest)
    ...
    public StandardDeviationCalculator(List<XYChart.Data<Number, Number>> graph) {
        if (graph.size() < 3) {
            throw new IllegalArgumentException("graph must contain at least two points (one for baseline and one latest)");
        }
        this.graph = graph;
    ...
    * Z = (R - μ) / s.
    *
    * <p>If the standard deviation is zero (all baseline values identical),
    * the method returns 0.0.</p>
    *
    ...
    */
     ...
     *
     * <p>When the standard deviation is zero:
    ...
    public double getPercentile() {
        ...
        if (sd == 0.0) {
            double mean = getMean();
            if (Double.compare(latest, mean) == 0) return 50.0;
     ...
     * Implementation based on a common numerical approximation (Abramowitz & Stegun).
     *
     * @param x input value
